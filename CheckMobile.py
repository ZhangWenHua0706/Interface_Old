...
def httpPost(account):
	...
	urlstring=cf.get('CheckMobile','url')
...
def CheckMobile(account):
	resultstr=httpPost(account)
	if '2000000' in resultstr:
		SaveResult.save_result('用户注册','检查号码是否存在','pass')
	else:
		SaveResult.save_result('用户注册','检查号码是否存在','fail')
if __name__=='__main__':
	CheckMobile('19933334411')